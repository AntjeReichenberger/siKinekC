...
function codeAddresses() {
	 for(var i =0; i < trackLocation.length; i++){
	   ...
	   //temp fixe remove "" condition
	   if(trackLocation[i].stateProv != "null" && trackLocation[i].stateProv != ""){
...
geocoder = new google.maps.Geocoder();
		geocoder.geocode( { 'address': trackRequests[requestNum-1]}, function(results, status) {
		  if (status == google.maps.GeocoderStatus.OK) {
			...
			//label value is +1 than it was for the label in the current response
