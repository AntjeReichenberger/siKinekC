...
function registerAjaxInvoker() {
	if(gmap.getBounds()!=null && gmap.getBounds()!='undefined' && jQuery('#directions').is(':hidden')){ //first load it will be undefined	
		...
		jQuery.ajax({
			type: "POST",
			url: "/wp-content/themes/platformpro/findkp_proxy.php",
			dataType:"json",
			data: {"criteria":jQuery('#criteria').val(),"displayCriteria":jQuery('#displayCriteria').val()
					,"radius":jQuery('#radius').val(),"maxLongitude":jQuery('#maxLongitude').val()
					,"minLongitude":jQuery('#minLongitude').val(),"maxLatitude":jQuery('#maxLatitude').val()
					,"minLatitude":jQuery('#minLatitude').val(),"searchPointLatitude":jQuery('#searchPointLatitude').val()
					,"searchPointLongitude":jQuery('#searchPointLongitude').val(),"borderProvinceId":jQuery('#borderProvinceId').val()},
...
function update(xhr) {
	...
	if(output.depots.length == 0) {
		...
		expanding = true;
		jQuery('#listWrapper1').css('height','325px');
		return;
	}		
	else {
		if(!expanding) {
			jQuery('#foundKp').show();
			jQuery('#listWrapper1').css('height','415px');
...
function showDirections(item) {
	...
	if(getRequestedPageName().toLowerCase()!="depotsearch.action"){
		...
		var directionsDest= jQuery('#directionsDestination').val();
		
		if(directionsDest){
			destination = directionsDest;	
		}
		else{
			destination = item.find('[name=address1HF]').val() + ', ' + item.find('[name=cityHF]').val() + ', ' + item.find('[name=stateHF]').val(); 
		}
...
function openInfoWindow(item){
	...
	var wrapper;
	if (getRequestedPageName().toLowerCase() != "find-a-kinekpoint-result") {
		/*
		var directionsLink = jQuery('<a></a>')
			.html('Get directions')
			.attr('href', '#')
			.addClass('directionLink')
			.click(function(e) {
				e.preventDefault();		
				//get the user profile address
				if(getRequestedPageName().toLowerCase()!="depotsearch.action"){
		...
		*/
		...
		var signUpImage = jQuery('<img></img>').attr('src','/wp-content/uploads/2010/11/SignUpNowSmall.png');
	...
	wrapper.css("line-height","1.2em");
	wrapper.css("font-size","11px");
	wrapper.css("color","#777");
...
function selectItem(item, showBubble) {
	...
	jQuery.ajax({
			type: "POST",
			url: "/wp-content/themes/platformpro/kpdetails_proxy.php",
			dataType:"json",
			data: {"depotId":depotId},
			success:function(data){
				var additionalDepotInfoJsonObj = data;
