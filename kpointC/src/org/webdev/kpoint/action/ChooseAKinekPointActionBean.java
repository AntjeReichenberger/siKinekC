...
@UrlBinding("/ChooseAKinekPoint.action")
public class ChooseAKinekPointActionBean extends SecureActionBean {
    ...
    public Resolution view() throws Exception {
    	...
    	if(!isSelectedKpFromMapList){
    		...
    		//Specific KP
    			...
    			if(depot != null) {
    				...
    				return UrlManager.getChooseAKinekPointResults();
    			}
    		}
    	}
    	...
    	if (getActiveUser().getState() != null) {
	    	borderProvinceId = getActiveUser().getState().getStateId();
	    	province = getActiveUser().getState().getName();
    	}
    	city = getActiveUser().getCity();
		return UrlManager.getChooseAKinekPoint();
	}
	
	@ValidationMethod(on="borderSearch")
	public void validateBorderSearch(ValidationErrors errors) throws Exception {
		//Border Search
		if (borderProvinceId != 0) {
	...
	@ValidationMethod(on="localSearch")
	public void validateLocalSearch(ValidationErrors errors) {
		radius = cityRadius; 
    ...
    public Resolution borderSearch() throws UnsupportedEncodingException {
       	return UrlManager.getChooseAKinekPointResults();
    }
    
    public Resolution localSearch() throws UnsupportedEncodingException {
       	return UrlManager.getChooseAKinekPointResults();
    }
